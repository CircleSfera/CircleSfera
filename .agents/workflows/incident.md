...
Respond to a production incident: stabilise first, explain second. Invoke with `/incident`.
...
2. Follow this playbook step by step, without skipping phases:
   - @/.ai/playbooks/incident.md
3. Consult these specialists in order, adopting the checks and hard rules of each for the part of
   the work it owns: incident-commander, observability, then backend or frontend by symptom,
   database, qa. Role definitions:
   - @/.ai/agents/README.md
...
Mitigation beats root cause. An image rollback does not revert the database, so check whether a
migration is involved before rolling back.
