...
Audit auth, authorization, data exposure, abuse surface, or personal-data handling. Invoke with
`/security-audit`.
...
2. Follow this playbook step by step, without skipping phases:
   - @/.ai/playbooks/security-audit.md
3. Consult these specialists in order, adopting the checks and hard rules of each for the part of
   the work it owns: security, backend, api, privacy-compliance, code-reviewer. Role definitions:
   - @/.ai/agents/README.md
...
Report findings with severity and evidence. Do not mix auditing and fixing in one pass unless a
finding is critical.
