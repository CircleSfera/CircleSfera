...
| **VIP / creator subscription** | **No `CreatorSubscription` Prisma model** (table removed). Do not invent it. Creator VIP billing, if present in product docs, is Stripe + `Transaction` application logic — verify controllers before coding. |
...
| **Admin** | Panel operators are `AdminIdentity` (+ roles/permissions), not platform `User.role` staff values ([ADR-0013](../../circlesfera-documentation/adr/0013-admin-panel-admin-identity.md)). |

## Models by domain

**Identity** `User`, `Profile`, `RefreshToken`, `Passkey`, `DeviceSignal`, `UserSettings`,
`DataExportRequest`, `CloseFriend`
...
`MessageUnlock`, `Transaction`, `WebhookEvent`, `Promotion`, `LiveGift`, `StripePayoutLog`

**Admin panel** `AdminIdentity`, `AdminRole`, `AdminPermission`, `AdminRolePermission`,
`AdminIdentityRole`, `AdminRefreshToken`, `AdminPasskey`, `AdminAuditLog`, `SystemSetting`

**Trust & safety** `Report`, `Appeal`, `ModerationRule`, `ModerationSignature`, `WhitelistEntry`
...
`UserExperiment`, `ProfileMetric`, `InteractionEvent`
...
`NotificationType`, `ReportReason`, `AdminAction`, `UserEventType`, `AdminIdentityStatus`,
`RuleAction`.
