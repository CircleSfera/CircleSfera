...
function updateDeps(pkgPath) {
  ...
  sections.forEach((section) => {
    ...
    names.forEach((name) => {
      try {
        console.log(
          `Installing ${name}@latest ${section === 'devDependencies' ? '--save-dev' : ''} --save-exact`,
        );
        execSync(
          `npm install ${name}@latest ${section === 'devDependencies' ? '--save-dev' : ''} --save-exact`,
          { stdio: 'inherit' },
        );
      } catch (e) {
...
updateDeps(
  path.resolve(__dirname, '..', '..', 'circlesfera-frontend', 'package.json'),
);
