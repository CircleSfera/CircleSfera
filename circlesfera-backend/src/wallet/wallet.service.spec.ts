...
describe('WalletService', () => {
  ...
  describe('sendTip', () => {
    ...
    it('should successfully tip, update balances and send notification', async () => {
      vi.mocked(prisma.wallet.findUnique).mockImplementation(((args: any) => {
        if (args.where.userId === 'user1') return Promise.resolve({ id: 'w1', userId: 'user1', balance: 100 });
        if (args.where.userId === 'user2') return Promise.resolve({ id: 'w2', userId: 'user2', balance: 0 });
      ...
      expect(prisma.wallet.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { userId: 'user2' },
        data: { earnedTokens: { increment: 40 } }
      }));
      expect(prisma.transaction.create).toHaveBeenCalled();
  ...
  describe('unlockPost', () => {
    ...
    it('should unlock post successfully', async () => {
      ...
      expect(prisma.wallet.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { userId: 'user2' },
        data: { earnedTokens: { increment: 80 } }
      }));
      expect(prisma.transaction.create).toHaveBeenCalled();
    });
  });
