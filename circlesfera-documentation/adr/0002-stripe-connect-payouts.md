...
- Admin `StripePayoutLog` is a copy of Connect `payout.created` / `payout.updated` / `payout.paid` / `payout.failed` / `payout.canceled` as Stripe sends them. Those events come from a **Connected-accounts** destination (see [stripe-connect-webhook](../runbooks/stripe-connect-webhook.md)), not from the platform Checkout webhook alone.
