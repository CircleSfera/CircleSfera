...
describe('exportStudioProject abort', () => {
  ...
  it('throws cancelled when abort fires while wasm URLs are still loading', async () => {
    vi.resetModules();
    vi.doMock('@ffmpeg/ffmpeg', () => ({
      FFmpeg: class {
        on() {}
    ...
    vi.doMock('@ffmpeg/util', () => ({
      ...
      toBlobURL: vi.fn(
        () =>
          new Promise<string>(() => {
            /* hang — same isolation as Playwright wasm route */
          }),
      ),
    }));
    ...
    const pending = exportStudioProject(project as any, {
