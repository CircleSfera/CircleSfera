    ...
    <nav className="fixed top-0 left-0 right-0 z-50 glass-panel border-b border-white/5 px-4 md:px-6 py-3 flex justify-between items-center backdrop-blur-xl bg-black/20">
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <span className="hidden sm:block text-xl font-black tracking-tight bg-clip-text text-transparent bg-linear-to-r from-brand-secondary via-brand-primary to-brand-blue animate-gradient-x bg-size-[200%_auto]">
            CircleSfera
          </span>
        </Link>
      </div>
      <div className="flex items-center gap-2 md:gap-6">
        <div className="hidden md:flex items-center gap-5">
          ...
          <Link
            to="/pricing"
            className={`text-xs font-semibold transition-colors tracking-wide uppercase ${isPricing ? 'text-white' : 'text-white/50 hover:text-white'}`}
          >
            {t.pricing.label}
          </Link>
        </div>
