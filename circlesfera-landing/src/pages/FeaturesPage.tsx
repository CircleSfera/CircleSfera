...
const FeaturesPage = () => {
  ...
  const featureAssets = [
    {
      id: 'visual-feed',
      icon: <Image key="visual-feed" className="w-6 h-6" />,
      color: 'from-brand-secondary to-orange-600',
    },
    {
      id: 'stories',
      icon: <Camera key="stories" className="w-6 h-6" />,
      color: 'from-brand-primary to-purple-600',
    },
    {
      id: 'frames',
      icon: <Play key="frames" className="w-6 h-6" />,
      color: 'from-brand-blue to-blue-600',
    },
    {
      id: 'chat',
      icon: <MessageCircle key="chat" className="w-6 h-6" />,
      color: 'from-emerald-500 to-teal-600',
    },
    {
      id: 'circles',
      icon: <Users key="circles" className="w-6 h-6" />,
      color: 'from-brand-accent to-amber-600',
    },
    {
      id: 'explore',
      icon: <Compass key="explore" className="w-6 h-6" />,
      color: 'from-cyan-500 to-blue-500',
    },
    {
      id: 'creator-studio',
      icon: <BarChart3 key="creator-studio" className="w-6 h-6" />,
      color: 'from-violet-500 to-purple-600',
    },
    {
      id: 'collections',
      icon: <Bookmark key="collections" className="w-6 h-6" />,
    ...
    <div className="min-h-screen relative overflow-hidden text-white font-sans selection:bg-brand-primary/30">
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16 relative z-10">
        <div className="text-center mb-12">
          <span className="text-brand-accent font-black text-[10px] tracking-[0.4em] uppercase mb-3 block opacity-60 animate-pulse">
            {t.featuresPage.diffLabel}
          </span>
          <h2 className="text-2xl md:text-3xl font-black tracking-tighter mb-4 drop-shadow-lg">
            {t.featuresPage.diffTitle}
          </h2>
          <p className="text-base text-white/30 max-w-xl mx-auto font-light leading-relaxed italic">
            {t.featuresPage.diffSubtitle}
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {t.featuresPage.differentiators.map(
            (d: { title: string; desc: string }, i: number) => {
              const glowColors = [
              ...
              return (
                <div
                  key={d.title}
                  className={`glass-panel p-6 rounded-3xl border border-white/5 transition-all duration-700 group hover:bg-white/4 relative overflow-hidden flex flex-col justify-end min-h-[220px] ${borderGlows[i]}`}
                >
                  <div
                    className={`absolute -top-20 -right-20 w-48 h-48 bg-linear-to-br ${glowColors[i]} rounded-full blur-[60px] group-hover:scale-125 transition-transform duration-1000`}
                  ></div>
                  <div className="relative z-10">
                    <div
                      className={`mb-4 transform group-hover:scale-105 transition-all duration-500 ease-out ${iconGlows[i]}`}
                    >
                      {diffIcons[i]}
                    </div>
                    <h3 className="text-lg font-black mb-2 tracking-tighter text-white">
                      {d.title}
                    </h3>
                    <p className="text-xs text-white/30 leading-relaxed font-light line-clamp-2 group-hover:text-white/50 transition-colors">
                      {d.desc}
                    </p>
                  </div>
                </div>
      ...
      <section className="max-w-4xl mx-auto px-6 py-10 relative z-10">
        <div className="text-center mb-10">
          <span className="text-brand-primary font-bold text-[10px] tracking-[0.3em] uppercase mb-2 block opacity-60">
            {t.featuresPage.featLabel}
          </span>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight">
            {t.featuresPage.featTitle}
          </h2>
        </div>

        <div className="space-y-8">
          {t.featuresPage.features.map(
            (
              f: {
                title: string;
                subtitle: string;
              ...
              return (
                ...
                >
                  <div
                    className={`h-1 bg-linear-to-r ${color} shadow-[0_0_20px_rgba(255,255,255,0.1)]`}
                  ></div>
                  <div
                    className={`p-5 md:p-6 flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-6 items-center`}
                  >
                    {/* Text content */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-4 mb-5">
                        <div
                          className={`w-10 h-10 rounded-xl bg-linear-to-br ${color} flex items-center justify-center text-white shadow-2xl transform group-hover:scale-105 transition-transform duration-500`}
                        >
                          {icon}
                        </div>
                        <div>
                          ...
                          <p className="text-[10px] text-white/40 font-light italic tracking-widest uppercase">
                            {f.subtitle}
                          </p>
                        </div>
                      </div>
                      <p className="text-base text-white/40 leading-relaxed mb-6 font-light italic">
                        {f.desc}
                      </p>
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {f.details.map((d: string) => (
                          <li
                            key={d}
                            className="flex items-start gap-2 p-2.5 rounded-xl bg-white/3 border border-white/5 transition-colors hover:bg-white/5"
                          >
                            <Heart className="w-3.5 h-3.5 mt-0.5 shrink-0 text-brand-secondary fill-brand-secondary/50 group-hover:animate-pulse" />
                            <span className="text-[11px] text-white/30 leading-relaxed font-light">
                              {d}
                            </span>
                          </li>
                        ))}
                    ...
                    <div className="flex-1 min-w-0 w-full">
                      <div
                        className={`w-full aspect-square md:aspect-16/10 rounded-3xl bg-black/40 border border-white/5 relative overflow-hidden shadow-2xl transition-shadow duration-700`}
                      >
                        {/* Ambient Glow */}
                        <div
                          className={`absolute -inset-10 bg-linear-to-br ${color} opacity-10 blur-3xl`}
                        ></div>
                        {/* Grid pattern */}
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-1.5 rounded-lg overflow-hidden">
          {[1, 2, 3, 4, 5, 6].map((id, i) => (
            <div
              key={`feed-${id}`}
              className={`aspect-square bg-linear-to-br ${i === 0 ? color : 'from-white/5 to-white/10'} rounded-sm ${i === 0 ? 'opacity-80' : 'opacity-30'}`}
            ></div>
          ))}
        </div>
      ...
      <div className="p-4 flex flex-col items-center justify-center h-full">
        <div className="flex gap-4 mb-4">
          {[1, 2, 3, 4, 5].map((id, i) => (
            ...
            >
              <div
                className={`w-12 h-12 rounded-full ${i === 0 ? `bg-linear-to-br ${color} shadow-[0_0_15px_rgba(131,58,180,0.3)]` : 'bg-white/10'} ${i === 0 ? 'ring-2 ring-brand-primary' : 'ring-1 ring-white/10'}`}
              ></div>
    ...
    frames: (
      <div className="p-4 flex items-center justify-center h-full relative">
        <div className="w-28 h-44 bg-white/5 rounded-xl border border-white/10 relative overflow-hidden">
          <div
            className={`absolute inset-0 bg-linear-to-b ${color} opacity-20`}
          ></div>
          ...
          <div className="absolute bottom-2 left-2 right-2">
            <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
              <div
                className={`h-full w-1/3 bg-linear-to-r ${color} rounded-full`}
              ></div>
            </div>
          </div>
        </div>
    ...
    chat: (
      <div className="p-4 space-y-3 flex flex-col justify-end h-full">
        <div className="flex justify-start">
          <div className="px-3 py-2 bg-white/5 rounded-xl rounded-bl-sm text-[10px] text-white/40 max-w-[60%]">
            {t.mockups.chat.msg1}
          </div>
        </div>
        <div className="flex justify-end">
          <div
            className={`px-3 py-2 bg-linear-to-r ${color} rounded-xl rounded-br-sm text-[10px] text-white/80 max-w-[60%]`}
          >
            {t.mockups.chat.msg2}
          </div>
        </div>
    ...
    circles: (
      <div className="p-4 space-y-2">
        {[
          ...
          <div
            key={circle.name}
            className={`flex items-center gap-3 p-2.5 rounded-xl ${i === 0 ? 'bg-white/5 border border-white/10' : ''}`}
          >
            <div
              className={`w-9 h-9 rounded-lg ${i === 0 ? `bg-linear-to-br ${color}` : 'bg-white/10'} flex items-center justify-center shrink-0`}
            >
            ...
            <div>
              ...
              <div className="text-[9px] text-white/20">
                {circle.count} {t.mockups.circles.members}
              </div>
            </div>
          </div>
        ))}
    ...
    explore: (
      <div className="p-3">
        <div className="grid grid-cols-3 gap-1.5">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((id, i) => (
            <div
              key={`explore-${id}`}
              className={`aspect-square rounded-sm ${i === 4 ? `bg-linear-to-br ${color} opacity-60 col-span-1 row-span-1` : 'bg-white/5'}`}
            ></div>
          ))}
        </div>
      </div>
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-end gap-1 h-20">
          {[
            ...
            <div
              key={`bar-${bar.id}`}
              className={`flex-1 rounded-t-sm ${i >= 10 ? `bg-linear-to-t ${color} opacity-80` : 'bg-white/10'}`}
              style={{ height: `${bar.h}%` }}
            ></div>
          ))}
        </div>
        <div className="flex justify-between text-[9px] text-white/20">
          <span>{t.mockups.months.jan}</span>
          <span>{t.mockups.months.jun}</span>
          <span>{t.mockups.months.dec}</span>
        </div>
      </div>
    ),
    collections: (
      <div className="p-4 grid grid-cols-2 gap-2">
        {[
          ...
          <div
            key={name}
            className={`aspect-4/3 rounded-lg ${name === t.mockups.collections.inspiration ? `bg-linear-to-br ${color} opacity-60` : 'bg-white/5'} flex items-end p-2`}
          >
