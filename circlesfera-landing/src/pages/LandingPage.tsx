...
import { Camera, MessageCircle, Users, Zap, Globe, Shield, Sparkles, ChevronDown, Mail, Send, Loader2, Play, Compass, Heart, Bookmark, BarChart3, Image, Lock } from 'lucide-react';
...
const LandingPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/whitelist/signup`, {
        method: 'POST',
      ...
      console.error(err);
      setStatus('error');
      const message = err instanceof Error ? err.message : t.whitelist.errorUnknown;
    ...
    <div className="min-h-screen relative overflow-hidden text-white font-sans selection:bg-brand-primary/30">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-28 pb-10 md:pt-32 md:pb-20 flex flex-col items-center text-center relative z-10">
        <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full glass-panel border border-white/20 mb-6 animate-float shadow-[0_0_20px_rgba(255,255,255,0.05)]">
          ...
          <span className="text-[10px] font-black tracking-[0.3em] uppercase text-white/90">{t.hero.badge}</span>
        </div>
        ...
        <a href="#whitelist" className="group relative px-8 py-4 bg-white text-black text-xs font-black rounded-2xl overflow-hidden transition-all hover:shadow-[0_0_50px_rgba(255,255,255,0.3)] hover:scale-[1.05] active:scale-95 shadow-2xl">
        ...
        <div className="mt-12 relative w-full max-w-3xl aspect-video glass-panel rounded-4xl border border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.8)] overflow-hidden group perspective-1000">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-12 z-20 text-left bg-linear-to-t from-black via-black/60 to-transparent">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <h3 className="text-3xl font-black tracking-tighter">{t.mockup.title}</h3>
                </div>
                <p className="text-white/40 max-w-lg text-base leading-relaxed font-light italic">{t.mockup.subtitle}</p>
            </div>
        </div>
      </main>
        ...
        <div className="flex gap-20 animate-marquee whitespace-nowrap items-center">
           {[...Array(6)].map((_, i) => (
             <React.Fragment key={i}>
               <span className="text-2xl font-black text-white/5 tracking-[0.3em] uppercase transition-all hover:text-brand-primary hover:scale-110 duration-700 cursor-default">{t.marquee.community}</span>
               <span className="text-xl font-bold text-white/5">•</span>
               <span className="text-2xl font-black text-white/5 tracking-[0.3em] uppercase transition-all hover:text-brand-secondary hover:scale-110 duration-700 cursor-default">{t.marquee.creativity}</span>
               <span className="text-xl font-bold text-white/5">•</span>
               <span className="text-2xl font-black text-white/5 tracking-[0.3em] uppercase transition-all hover:text-brand-blue hover:scale-110 duration-700 cursor-default">{t.marquee.connection}</span>
               <span className="text-xl font-bold text-white/5">•</span>
               <span className="text-2xl font-black text-white/5 tracking-[0.3em] uppercase transition-all hover:text-brand-accent hover:scale-110 duration-700 cursor-default">{t.marquee.innovation}</span>
               <span className="text-xl font-bold text-white/5">•</span>
             </React.Fragment>
           ))}
        </div>
      ...
      <section className="py-10 relative bg-white/1">
         <div className="max-w-2xl mx-auto px-6">
            <div className="text-center mb-10">
                <span className="text-brand-accent font-bold text-[10px] tracking-[0.3em] uppercase mb-2 block opacity-60">{t.faq.label}</span>
                <h2 className="text-2xl md:text-3xl font-bold tracking-tight">{t.faq.title}</h2>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 flex flex-col items-center">
          ...
          <div className="w-full max-w-lg glass-panel p-8 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden bg-black/40 backdrop-blur-2xl">
              ...
              <div className="text-center mb-8">
                <h2 className="text-2xl md:text-3xl font-black mb-2 tracking-tighter">{t.whitelist.title}</h2>
              ...
              {status === 'success' ? (
                  <div className="py-8 text-center relative z-10">
                      ...
                      <h3 className="text-xl font-bold mb-2">{t.whitelist.successTitle}</h3>
                      <p className="text-sm text-white/50 mb-6">{t.whitelist.successMessage}</p>
                      <button 
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
                      <div className="grid grid-cols-1 gap-4">
                          <div className="text-left">
                              <label htmlFor="name" className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-1.5 ml-1 block">{t.whitelist.nameLabel}</label>
                              <input 
                                  type="text" 
                                  id="name" 
                                  value={name}
                          ...
                          <div className="text-left">
                              <label htmlFor="email" className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-1.5 ml-1 block">{t.whitelist.emailLabel}</label>
                      ...
                      {errorMessage && <p className="text-red-400 text-xs mt-2 px-1">{errorMessage}</p>}
                      ...
                      <p className="text-[10px] text-white/20 text-center uppercase tracking-[0.2em] font-medium mt-4">{t.whitelist.reserveSpot}</p>
                  </form>
              )}
          </div>
        </div>
...
const FeaturesShowcase = () => {
    const { t } = useLanguage();
    const featureData = [
        { id: 'visual-feed', icon: <Image className="w-5 h-5" />, color: "from-brand-secondary to-orange-600", span: "md:col-span-2" },
        { id: 'stories', icon: <Camera className="w-5 h-5" />, color: "from-brand-primary to-purple-600", span: "" },
        { id: 'frames', icon: <Play className="w-5 h-5" />, color: "from-brand-blue to-blue-600", span: "" },
        { id: 'chat', icon: <MessageCircle className="w-5 h-5" />, color: "from-emerald-500 to-teal-600", span: "md:col-span-2" },
        { id: 'circles', icon: <Users className="w-5 h-5" />, color: "from-brand-accent to-amber-600", span: "" },
        { id: 'explore', icon: <Compass className="w-5 h-5" />, color: "from-cyan-500 to-blue-500", span: "" },
        { id: 'creator-studio', icon: <BarChart3 className="w-5 h-5" />, color: "from-violet-500 to-purple-600", span: "" },
        { id: 'collections', icon: <Bookmark className="w-5 h-5" />, color: "from-pink-500 to-rose-600", span: "" },
    ];
    ...
    return (
        <section className="py-10 relative">
            <div className="max-w-4xl mx-auto px-6">
                <div className="text-center mb-10">
                    <span className="text-brand-accent font-bold text-[10px] tracking-[0.3em] uppercase mb-2 block opacity-60">{t.showcase.label}</span>
                    <h2 className="text-2xl md:text-3xl font-bold mb-3 tracking-tight">{t.showcase.title}</h2>
                    <p className="text-sm text-white/30 max-w-md mx-auto font-light italic">{t.showcase.subtitle}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {t.showcase.features.map((f: { title: string; desc: string }, i: number) => (
                        <Link
                        ...
                        >
                            <div className={`absolute top-0 left-0 w-full h-0.5 bg-linear-to-r ${featureData[i].color} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
                            
                            <div className="flex items-start gap-4">
                                <div className={`w-10 h-10 rounded-lg bg-linear-to-br ${featureData[i].color} flex items-center justify-center shrink-0 text-white shadow-lg group-hover:scale-110 transition-transform duration-500`}>
                                    {featureData[i].icon}
                                </div>
                                <div className="min-w-0">
                                    <h3 className="text-sm font-bold mb-1 tracking-tight group-hover:text-white transition-colors">{f.title}</h3>
                                    <p className="text-xs text-white/30 leading-relaxed font-light">{f.desc}</p>
                                    <span className="text-[10px] text-brand-primary font-semibold mt-2 inline-block opacity-0 group-hover:opacity-100 transition-opacity duration-300">{t.showcase.learnMore}</span>
                                </div>
                            </div>
                        </Link>
                    ))}
                ...
                <div className="mt-8 flex items-center justify-center gap-3 text-white/20">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em]">{t.showcase.securityBadge}</span>
...
const InteractiveFeatures = () => {
    ...
    return (
        <section className="py-10 max-w-5xl mx-auto px-6">
             <div className="text-center mb-10">
                 <span className="text-brand-primary font-bold text-[10px] tracking-[0.3em] uppercase mb-2 block opacity-60">{t.interactive.label}</span>
                 <h2 className="text-2xl md:text-3xl font-bold tracking-tight">{t.interactive.title}</h2>
             </div>
            <div className="glass-panel rounded-2xl p-1.5 border border-white/5 max-w-4xl mx-auto flex flex-col lg:flex-row overflow-hidden min-h-[450px] md:min-h-[380px] shadow-2xl">
                ...
                <div className="w-full lg:w-1/3 p-4 flex flex-row lg:flex-col gap-2 md:gap-3 border-b lg:border-b-0 lg:border-r border-white/5 relative z-20 overflow-x-auto no-scrollbar">
                    <h3 className="hidden lg:block font-bold mb-2 text-white/30 uppercase tracking-[0.3em] text-[10px]">{t.interactive.navLabel}</h3>
                    {tabKeys.map(id => (
                        <button type="button"
                            key={id}
                        ...
                        >
                            {activeTab === id && <div className="absolute left-0 lg:left-0 top-0 lg:top-0 bottom-0 lg:bottom-0 w-1 lg:w-1 h-0.5 lg:h-full bg-brand-primary"></div>}
                            <div className={`w-8 h-8 md:w-9 md:h-9 rounded-lg flex items-center justify-center transition-all duration-500 ${activeTab === id ? 'bg-brand-primary text-white shadow-[0_0_15px_rgba(131,58,180,0.35)]' : 'bg-white/5 text-gray-500 group-hover:text-white group-hover:scale-110'}`}>
                                {tabIcons[id]}
                            </div>
                            <div>
                                <span className={`block font-black text-xs md:text-sm transition-colors ${activeTab === id ? 'text-white' : 'text-gray-500 group-hover:text-white'}`}>{t.interactive.tabs[id].label}</span>
                            </div>
                        </button>
                    ))}
                </div>
                ...
                {/* Content Area */}
                <div className="w-full lg:w-2/3 p-6 md:p-10 relative flex items-center justify-center bg-black/20 overflow-hidden min-h-[250px]">
                    ...
                    {tabKeys.map(id => (
                        ...
                        >
                            ...
                            <h2 className="text-2xl md:text-4xl font-black mb-3 md:mb-4 tracking-tighter leading-tight md:leading-none">{t.interactive.tabs[id].title}</h2>
