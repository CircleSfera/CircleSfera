...
test.describe('Happy Path', () => {
  test('should allow a user to register, login, and see the feed', async ({ page }) => {
    ...
    page.on('response', async response => {
      if (response.status() >= 400 && response.url().includes('/api/v1/')) {
        try {
          ...
          console.log(`NETWORK ERROR RESPONSE: ${response.status()} ${response.url()}:`, JSON.stringify(body, null, 2));
        } catch (e) {
          console.log(`NETWORK ERROR RESPONSE: ${response.status()} ${response.url()} (could not parse body)`);
        }
      }
    });
    ...
    const createLink = page.locator('nav[aria-label="Main Navigation"] a[href="/create"]').first();
    ...
    await expect(page.getByText('Select from computer')).toBeVisible({ timeout: 15000 });
    ...
    await fileInput.setInputFiles({
      name: 'test-image.png',
      mimeType: 'image/png',
      buffer: Buffer.from('fake-image-data-at-least-some-bytes-to-be-valid-ish'),
    });
    ...
    await expect(page.getByText('Profile updated successfully!')).toBeVisible({ timeout: 20000 });
