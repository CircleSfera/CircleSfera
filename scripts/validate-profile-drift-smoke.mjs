...
async function fetchJson(path, { method = 'GET', cookies, body, csrfToken } = {}) {
...
async function loadAdminTotpSecret() {
  if (process.env.ADMIN_E2E_TOTP_SECRET) return process.env.ADMIN_E2E_TOTP_SECRET;
  try {
...
async function adminLogin() {
  ...
  if (j1.status === 'MFA_REQUIRED' || j1.status === 'MFA_SETUP_REQUIRED') {
    ...
    if (!r2.ok) throw new Error(`admin MFA HTTP ${r2.status}: ${JSON.stringify(j2)}`);
...
async function userLogin() {
  ...
  if (!res.ok) throw new Error(`user login HTTP ${res.status}: ${JSON.stringify(json)}`);
...
async function loginPlatformUser(email, password = USER_PASSWORD) {
  ...
  if (!res.ok) {
    throw new Error(`login ${email} HTTP ${res.status}: ${JSON.stringify(json)}`);
  }
...
async function validateWithFixtures(adminCookies, userCookies) {
  ...
  if (postId) {
    ...
    if (!cr.ok) {
      ...
      const row = (json?.data ?? []).find((r) => r.id === cj?.id || r.details === marker);
      if (!res.ok) record('admin reports reporter.profile (fixture)', 'FAIL', `HTTP ${res.status}`);
      else if (!row) record('admin reports reporter.profile (fixture)', 'FAIL', 'report missing in admin list');
      else if (!reporterShapeOk(row)) record('admin reports reporter.profile (fixture)', 'FAIL', 'missing reporter.profile.username');
      else record('admin reports reporter.profile (fixture)', 'PASS', `@${row.reporter.profile.username}`);
    }
  } else {
  ...
  if (!ownerRow.includes('|')) {
    record('interactive qna/:id (fixture)', 'SKIP', 'no post without existing QnA box');
  } else {
    ...
    try {
      ...
      if (!boxRes.ok) {
        record('interactive qna/:id (fixture)', 'FAIL', `create HTTP ${boxRes.status}`);
      } else {
        ...
        if (!ansRes.ok) {
          record('interactive qna/:id (fixture)', 'FAIL', `answer HTTP ${ansRes.status}`);
        } else {
          const { res: getRes, json: qna } = await fetchJson(`/interactive/qna/${box.id}`);
          if (!getRes.ok) record('interactive qna/:id (fixture)', 'FAIL', `GET HTTP ${getRes.status}`);
          else {
            const ans = qna?.answers?.[0];
            if (!ans?.user?.username || ans.user.username === 'usuario') {
              record('interactive qna/:id (fixture)', 'FAIL', `bad username: ${ans?.user?.username}`);
            } else {
              record('interactive qna/:id (fixture)', 'PASS', `@${ans.user.username}`);
            }
          }
        }
      }
  ...
  if (!recipientId) {
    ...
    if (!sendRes.ok) {
      record('chat edit message (fixture)', 'SKIP', `send HTTP ${sendRes.status}`);
    } else {
      const { res: editRes, json: edited } = await fetchJson(`/chat/messages/${sent.id}`, {
        method: 'PUT',
        cookies: csrfCookies,
        csrfToken,
        body: { content: `${marker}-edited` },
      });
      if (!editRes.ok) record('chat edit message (fixture)', 'FAIL', `edit HTTP ${editRes.status}`);
      else if (!edited?.sender?.username) {
        record('chat edit message (fixture)', 'FAIL', 'missing sender.username on edit response');
      } else {
        record('chat edit message (fixture)', 'PASS', `@${edited.sender.username}`);
      }
    }
  }
}
...
async function main() {
  ...
  // Admin reports
  {
    ...
    if (!res.ok) {
      ...
      const human = rows.filter((r) => !r.details?.includes?.('[AI Automated Flag]'));
      const bad = human.filter((r) => r.reporter && !reporterShapeOk(r));
      if (bad.length) {
        record('admin reports reporter.profile', 'FAIL', `${bad.length} row(s) missing profile.username`);
      } else if (human.length === 0) {
        record('admin reports reporter.profile', 'SKIP', 'no human reports in sample');
      } else {
        record('admin reports reporter.profile', 'PASS', `${human.length} checked`);
      }
    }
  }
  ...
  // Admin transactions
  {
    ...
    if (!res.ok) {
      ...
      if (bad.length) {
        record('admin transactions sender.profile', 'FAIL', `${bad.length} still profiles[] shape`);
      } else if (rows.length === 0) {
        record('admin transactions sender.profile', 'SKIP', 'empty ledger');
      } else {
        record('admin transactions sender.profile', 'PASS', `${rows.length} row(s)`);
      }
    }
  }
  ...
  for (const [path, label] of adminListChecks) {
    ...
    const bad = list.filter((row) => {
      if (label === 'live streams') return !row.host?.profile?.username && row.host;
      return row.user !== undefined && row.user !== null && !hasAdminProfile(row);
    });
    if (bad.length) {
      record(`admin ${label} user.profile`, 'FAIL', `${bad.length}/${list.length} missing`);
    } else {
  ...
  // Appeals
  {
    ...
    if (!res.ok) {
      ...
      const rows = json?.data ?? [];
      const bad = rows.filter((a) => a.user && !a.user.profile?.username && !a.user.email);
      if (bad.length) record('admin appeals user.profile', 'FAIL', `${bad.length} bad`);
      else if (!rows.length) record('admin appeals user.profile', 'SKIP', 'empty');
      else record('admin appeals user.profile', 'PASS', `${rows.length} row(s)`);
    }
  }

  // User dossier
  {
    const { res: listRes, json: listJson } = await fetchJson('/admin/users?limit=1', {
      cookies: adminCookies,
    });
    if (!listRes.ok) {
      ...
      if (!userId) {
        ...
        if (!res.ok) record('admin user dossier profile', 'FAIL', `HTTP ${res.status}`);
        else if (!json?.profile?.username) record('admin user dossier profile', 'FAIL', 'missing profile.username');
        else record('admin user dossier profile', 'PASS', `@${json.profile.username}`);
      }
    }
  }
  ...
  if (userCookies) {
    const { res, json } = await fetchJson('/monetization/transactions?limit=10', {
      cookies: userCookies,
    });
    if (!res.ok) {
      ...
      const rows = json?.data ?? [];
      record('creator monetization transactions', 'PASS', `${rows.length} row(s), no 500`);
      const bad = rows.filter((tx) => tx.sender?.profiles && !tx.sender?.profile);
      if (bad.length) record('creator tx sender.profile shape', 'FAIL', `${bad.length} unmapped`);
